...
// ── Couleurs dégradé tracé (identique au mobile) ───────────────────────────────
...
class _LivePageState extends State<LivePage> {
  ...
  String? errorMessage;
  String? errorDetails; // exception technique dépliable
  ...
  // ── Ticker "il y a X min" (1 s, sans appel réseau) ───────────────────────────
  Timer? _tickerTimer;
  ...
  String _formatAvgSpeed() {
    final secs = _rideDuration.inSeconds.toDouble();
    if (secs <= 0 || _totalDistanceMeters <= 0) return '-- km/h';
    final kmh = (_totalDistanceMeters / 1000) / (secs / 3600);
  ...
  // ── Recentrer sur position courante ──────────────────────────────────────────
  void _recenter() {
  ...
  Future<void> loadLastPosition({bool manual = false}) async {
    ...
    try {
      ...
      // Fit bounds au premier chargement, sinon juste recentrer
    ...
    return FlutterMap(
      ...
      children: [
        TileLayer(
          ...
          urlTemplate: kMapStyles[_mapStyleIndex]['url'] as String,
          subdomains:
              kMapStyles[_mapStyleIndex]['subdomains'] as List<String>,
          maxZoom:
              (kMapStyles[_mapStyleIndex]['maxZoom'] as int).toDouble(),
        ...
        MarkerLayer(
          markers: [            // Point de départ (orange)
              ...
              Marker(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: const Color(0xFFFF8A00), width: 2),
                    boxShadow: [
                      BoxShadow(
            ...
            // Point d'arrivée / dernier point (violet) — seulement si différent du départ
            if (points.length > 1)
              Marker(
                point: points.last,
                width: 26,
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle,
                    border: Border.all(
                        color: const Color(0xFF6D28D9), width: 2),
                    boxShadow: [
                      BoxShadow(
  ...
  Widget _buildStatsCard() {
    ...
    // Couleur alerte si dernière position > 5 min
    final staleMins = _sinceLastUpdate.inMinutes;
    final staleColor = staleMins >= 10
        ? const Color(0xFFF87171)   // rouge
        : staleMins >= 5
            ? const Color(0xFFFACC15) // jaune
            : const Color(0xFFFF8A00); // orange normal
                  ...
                  children: [
                    ...
                    Expanded(
                        child: _StatItem(
                            ...
                            label: 'Dernière pos.',
                            value: _formatSinceLastUpdate(),
                            valueColor: staleColor)),
                  ],
                ),
              ],
            )
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: getStatusColor().withValues(alpha: 0.45), width: 2),
            ),
            child: Icon(getStatusIcon(),
                color: getStatusColor(), size: isMobile ? 36 : 54),
          ),
  ...
  Widget _buildErrorScreen() {
    ...
    final IconData icon = isMissingCode
        ? Icons.link_off
        : isNoPosition
            ? Icons.location_off
            : Icons.cloud_off;

    final Color iconBg = isMissingCode
        ? const Color(0xFF2D2010)
        : isNoPosition
            ? const Color(0xFF1A2030)
            : const Color(0xFF2A1A1A);

    final Color iconColor = isMissingCode
        ? const Color(0xFFFACC15)
        : isNoPosition
            ? const Color(0xFF60A5FA)
            : const Color(0xFFF87171);

    final String title = isMissingCode
        ? 'Lien de partage invalide'
        : isNoPosition
            ? 'Aucune position disponible'
            : 'Impossible de charger le ride';
    ...
    final String subtitle = isMissingCode
        ...
        : isNoPosition
            ? 'Le ride existe mais aucune position GPS n\'a encore été enregistrée. Attends quelques secondes et rafraîchis la page.'
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Étapes (uniquement si code manquant)
                if (isMissingCode)
                  Container(
                    width: double.infinity,
                    ...
                    child: Column(
                      ...
                      children: [
                        const Text(
                          'Comment obtenir un lien valide ?',
                          style: TextStyle(
                ...
                // Bouton réessayer (erreur système ou pas de position)
                if (!isMissingCode) ...[
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton.icon(
                ...
                // Panneau dépliable détails techniques
                if (isSystemError && errorDetails != null) ...[
                  const SizedBox(height: 16),
                  GestureDetector(
                    onTap: () => setState(
                        () => _errorDetailsExpanded = !_errorDetailsExpanded),
                    child: Container(
                      width: double.infinity,
                      decoration: BoxDecoration(
                        color: const Color(0xFF1B1B1B),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Spinner discret en cours de refresh
          if (isRefreshing)
            const Padding(
            ...
            Expanded(
              child: Stack(
                children: [
                  ...
                  // Switcher style carte (bas gauche)
                  Positioned(
                    bottom: 16,
                    left: 16,
